...
export default function UploadPage() {
  ...
  const upload = async () => {
    ...
    try { const { data } = await api.post("/reports", body, { headers: { "Content-Type": "multipart/form-data" } }); setStatus("complete"); setMessage(data.message || "Stored privately. Local extraction will begin after security checks."); }
    catch (error: unknown) { const detail = typeof error === "object" && error !== null && "response" in error ? (error as { response?: { data?: { detail?: string } } }).response?.data?.detail : null; setStatus("idle"); setMessage(detail || "The report was not uploaded. Please try again."); }
  };
